...
export function formatText(result) {
  ...
  for (const catalog of result.catalogs) {
    ...
    for (const query of catalog.queries) lines.push(
      `  ${JSON.stringify(query.query)}: ${query.returned} results; ${JSON.stringify(query.searchMethod)}; ${query.pages} pages, ${query.scanned} scanned, ${query.skipped} invalid; bounded=${query.truncated}; errors=${JSON.stringify(query.errors)}`);
  }
  for (const candidate of result.candidates) {
    lines.push(`\n${JSON.stringify(candidate.identity)}; matches ${JSON.stringify(candidate.matchedQueries)}`);
    for (const observation of candidate.observations) {
      lines.push(`  ${observation.catalog}: ${JSON.stringify(observation.name)} (${observation.kind})`);
      lines.push(`  ${JSON.stringify(observation.description)}`);
      lines.push(`  Payment terms (raw asset units): ${JSON.stringify(observation.paymentOptions)}; activity (catalog-reported): ${JSON.stringify(observation.activity)}`);
    }
  }
